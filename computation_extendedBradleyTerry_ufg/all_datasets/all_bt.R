...
data$generation.dataset <-  paste(data$id, data$Dataset)
data$model.specification <- paste(data$Model, data$Method)

data <- data[ ,c("generation.dataset",
                 "model.specification",
...
# list_graph <- readRDS("alldatasetsbt_list_graph.RDS")
