...
data_subset_name <-  "wikinews"


# prefix of stored files
prefix_files <- "wikinews"
...
# list_graph <- readRDS("wikinewsbt_list_graph.RDS")
...
# print(xtable((df_edge_exist[max_index$ix[which(max_index$x == max_index$x[1])], ]), digits = 0), include.rownames=FALSE)
...
result_glm <- sort(unlist(worth), decreasing = TRUE)
result_glm

print(xtable(data.frame(x = names(result_glm), y = as.numeric(result_glm)),
             digits = 50),
